...
async def get_buying_list(message):
    for number in range(1,7):
        photo = f"photo_{number}.jpg"
        with open(photo, 'rb') as img:
            text_about = f"Product{number} | Описание: {opis[number]} | Цена: {number*100}"
            await message.answer(text=text_about)
            await message.answer_photo(img, f"№ {number}  {opis[number]} Цена: {number*100}")
