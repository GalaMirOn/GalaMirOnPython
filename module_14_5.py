...
async def set_username(message, state):
    await state.update_data(data_username=message.text)
...
async def set_username(message, state):
    ...
    set_username = data['data_username']
    set_email = data['data_email']
    set_age = int(data['data_age'])
    #print(set_username, set_email, set_age)
    add_user(set_username, set_email, set_age)
    await message.answer('Регистрация прошла успешно! Добро пожаловать!', reply_markup=kb)
    await state.finish()
